...
async function run() {
  ...
  const token = await ethers.getContractAt(
    '@pooltogether/yield-source-interface/contracts/test/ERC20Mintable.sol:ERC20Mintable',
    await yieldSource.depositToken(),
  );
  const prizePool = await ethers.getContract('PrizePool-1');
  ...
  const depositTx = await prizePool.depositToAndDelegate(
    signers[0].address,
    amount,
    signers[0].address,
  );
