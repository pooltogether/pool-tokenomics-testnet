...
async function run() {
  ...
  const ticket1 = await ethers.getContract('Ticket1')
  ...
  const userPicks = await drawCalculator.calculateUserPicks(ticket1.address, signers[0].address, [newestDraw.drawId])
