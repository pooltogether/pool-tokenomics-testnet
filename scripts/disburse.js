...
async function run() {
  ...
  for (let index = 0; index < mintTokenTo.length; index++) {
    ...
    console.log(
      chalk.dim(
        `Minting ${ethers.utils.formatEther(amount)} deposit tokens to ${mintTokenTo[index]}...`,
      ),
    );
    await token.mint(mintTokenTo[index], amount);
    console.log(
      chalk.dim(
        `Transferring ${ethers.utils.formatEther(amount)} POOL tokens to ${mintTokenTo[index]}...`,
      ),
    );
