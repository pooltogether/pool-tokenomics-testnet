...
async function arb(streamId, marketRate) {
  ...
  if (currentProfit.gt(ethers.utils.parseEther('0.1'))) {
    console.log(
      dim(
        `${streamPrefix} Prize distributor balance: ${ethers.utils.formatEther(
          await pool.balanceOf(prizeDistributor.address),
        )}`,
      ),
    );
    ...
    console.log(
      green(
        `${streamPrefix} amount in: ${ethers.utils.formatEther(
          currentAmount,
        )} amount out: ${ethers.utils.formatEther(
          amountOut,
        )} with profit ${ethers.utils.formatEther(currentProfit)}...`,
      ),
    );
    ...
    console.log(
      dim(
        `${streamPrefix} Arbed!  Prize distributor new balance: ${ethers.utils.formatEther(
          await pool.balanceOf(prizeDistributor.address),
        )}`,
      ),
    );
    ...
    console.log(
      yellow(
        `${streamPrefix} Best trade: ${ethers.utils.formatEther(
          currentAmount,
        )} with profit ${ethers.utils.formatEther(currentProfit)}`,
      ),
    );
